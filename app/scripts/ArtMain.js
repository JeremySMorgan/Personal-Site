...
function rangeOutput(n,min,max){
	//NewValue = (((OldValue - OldMin) * (NewMax - NewMin)) / (OldMax - OldMin)) + NewMin

	return (((n) * (max - min)) / (100)) + min;
}



function xcircle(time,rad){
	return  rad*Math.cos(toDegrees(time));
}

function ycircle(time,rad){
	return rad*Math.sin(toDegrees(time));
}

function xPath(time,rad){
	return rad*Math.cos(toDegrees(time))
}

function yPath(time,rad){
	return rad*Math.sin(toDegrees(time))
}
